...
class ZeroDataHandler:
    ...
    def handle_zero_data_account(self, primary_username: str, secondary_usernames: List[str], 
        ...
        try:
            # ------------------------------------------------------------------
            # BULLETPROOF VALIDATION: Comprehensive data availability check
            # ------------------------------------------------------------------
            logger.info("🔍 BULLETPROOF: Conducting comprehensive data availability analysis")
            
            # Check vector database status
            vector_db_docs = 0
            if self.vector_db and hasattr(self.vector_db, 'get_count'):
                try:
                    vector_db_docs = self.vector_db.get_count()
                    logger.info(f"📊 Vector database contains {vector_db_docs} documents for RAG analysis")
                except Exception as e:
                    logger.warning(f"⚠️ Could not get vector DB count: {str(e)}")
            
            # Validate competitor data availability
            competitor_data_validation = self._validate_competitor_data_availability(
                primary_username, secondary_usernames, platform, available_competitor_data, vector_db_docs
            )
    ...
    def _validate_competitor_data_availability(self, primary_username: str, secondary_usernames: List[str],
                                             platform: str, available_competitor_data: Dict,
                                             vector_db_docs: int) -> Dict:
        """
        BULLETPROOF: Validate competitor data availability across all sources.
        Provides comprehensive analysis of data availability for robust decision making.
        """
        validation_result = {
            'total_competitors': len(secondary_usernames),
            'available_data_competitors': 0,
        ...
        try:
            # Validate available competitor data
            if available_competitor_data:
                for competitor, data in available_competitor_data.items():
                    posts = data.get('posts', [])
                    if posts and len(posts) > 0:
                        validation_result['available_data_competitors'] += 1
                        validation_result['total_posts_available'] += len(posts)
            ...
            # Validate vector database status
            if vector_db_docs > 0:
                validation_result['vector_db_status'] = 'populated'
            elif vector_db_docs == 0:
                validation_result['vector_db_status'] = 'empty'
            
            # Determine validation status
            if validation_result['available_data_competitors'] > 0 or vector_db_docs > 0:
                validation_result['validation_passed'] = True
                validation_result['recommendations'].append(f"Proceed with competitor data RAG - {validation_result['available_data_competitors']} competitors available")
            else:
                validation_result['validation_passed'] = False
                validation_result['recommendations'].append("No competitor data available - consider skipping RAG or using fallback")
            
            logger.info(f"🔍 VALIDATION RESULTS for @{primary_username}:")
            logger.info(f"  - Competitors with data: {validation_result['available_data_competitors']}/{validation_result['total_competitors']}")
            logger.info(f"  - Total posts available: {validation_result['total_posts_available']}")
            logger.info(f"  - Vector DB status: {validation_result['vector_db_status']} ({vector_db_docs} docs)")
            logger.info(f"  - Validation passed: {validation_result['validation_passed']}")
            
            return validation_result
            
        except Exception as e:
            logger.error(f"❌ Validation failed for @{primary_username}: {str(e)}")
            validation_result['validation_passed'] = False
                ...
                try:
                    ...
                    for i, post in enumerate(all_competitor_posts):
                        ...
                        self.vector_db.add_post(
                            post_id=f"temp_competitor_{primary_username}_{post['username']}_{i}",
                        ...
                        self.vector_db.add_post(
                            post_id=f"temp_competitor_nc_{primary_username}_{post['username']}_{i}",
                            content=post['content'],
                            metadata={
                                'username': post['username'],
                                'platform': platform,
                        ...
                        self.vector_db.add_post(
                            post_id=f"shadow_{primary_username}_{post['username']}_{i}",
                            content=post['content'],
                            metadata={
...
{{ ... }}
        ...
        try:
            ...
            recommendation = self.rag.generate_recommendation(
                primary_username,  # Keep original username for identification
    ...
    def _generate_gemini_only_recommendation(self, primary_username: str, platform: str, 
        ...
        try:
            ...
            raise Exception(f"Failed to generate real content for {primary_username} - all generation methods failed. Check data availability and API quota.")
