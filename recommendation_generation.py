...
import pandas as pd
# from news_api import NewsAPIClient  # REMOVED: Module doesn't exist
...
class RecommendationGenerator:
    ...
    def __init__(self, rag=None, time_series=None, templates=CONTENT_TEMPLATES):
        ...
        if self.rate_limiter:
            logger.info(f"🕒 Using RAG rate limiter: min delay={self.rate_limiter.min_delay:.1f}s")
            
        # DISABLED: Vector database auto-population - using only scraped data
        logger.info("🔍 RecommendationGenerator using scraped data only (no auto-population)")
    ...
    def generate_unified_content_plan(self, primary_username, secondary_usernames, query, is_branding=True, platform="instagram"):
        ...
        for attempt in range(max_retries):
            try:
                ...
                required_modules = [intelligence_type, "tactical_recommendations", "trending_hashtags", "trending_hashtags_reason"]  # REMOVED next_post_prediction
                ...
                formatted_result = {
                    ...
                    'trending_hashtags': unified_result["trending_hashtags"],
                    'trending_hashtags_reason': unified_result["trending_hashtags_reason"],
                ...
                try:
                    ...
                    if i < len(selected_topics) - 1:
                        ...
                        if hasattr(self, 'rate_limiter') and self.rate_limiter:
                            # Use the rate limiter's min delay as guidance
                            delay = max(30, self.rate_limiter.min_delay)
    ...
    def generate_next_post_prediction(self, posts, account_analysis=None, platform="instagram"):
        ...
        """
        logger.warning("🚨 DEPRECATED: generate_next_post_prediction called - should use dedicated module instead")
    ...
    def _determine_branding_from_analysis(self, account_analysis):
        ...
        # CRITICAL FIX: Use explicitly passed is_branding value if present
        if 'is_branding' in account_analysis:
            is_branding = account_analysis['is_branding']
            logger.info(f"🎯 EXPLICIT is_branding={is_branding} found in account analysis")
            return is_branding
            
        # Fallback: Check explicit account type
    ...
    def generate_news_based_content(self, account_analysis, platform="instagram"):
        ...
        try:
            ...
            # Fetch news with the intelligent query - USING INTELLIGENT NEWS MODULE
            try:
                # Import our intelligent news module
                from intelligent_news_for_you import IntelligentNewsForYouModule
                
                # Create mock dependencies for the news module
                class MockConfig:
                    def __init__(self):
                        self.NEWSDATA_API_KEY = "pub_81555ab19b0046a7b3d947cddc59fe99c9146"
                ...
                # Convert query components to hashtags for the news module
                hashtags = [f"#{term}" for term in news_query_components[:5]]
                
                # Generate news using our intelligent hashtag conversion
                news_results = news_module.generate_news_for_account_sync(
                    username=username,
                    platform=platform,
                    user_posts=None
                )
                ...
                if news_results:
                    ...
                    formatted_articles = []
                    for result in news_results[:3]:  # Limit to 3 articles
                        formatted_article = {
                            'title': result.get('breaking_news_summary', 'Latest news update'),
                            'description': result.get('breaking_news_summary', 'Stay informed with the latest developments.'),
                            'url': result.get('source_url', '#'),
                            'source': 'Intelligent News Module'
                        }
                ...
                logger.warning(f"Intelligent news module failed, using fallback: {str(news_error)}")
                # Fallback: return generic news structure
                fallback_articles = [
                    {
                        'title': f'Latest {news_query_components[0] if news_query_components else "trending"} news',
                        'description': f'Stay updated with the latest developments in {news_query_components[0] if news_query_components else "current events"}.',
                        'url': '#',
