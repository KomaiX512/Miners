...
class R2StorageManager:
    ...
    def read_json(self, key, bucket='tasks'):
        """Read and parse a JSON object from the specified bucket.
        ...
        Returns:
            dict | None: Parsed JSON dictionary on success, None on error
        """
        try:
            response = self.client.get_object(Bucket=bucket, Key=key)
            body_bytes = response['Body'].read()
            # Attempt utf-8 first; fallback to latin-1 if needed to avoid hard failures
            try:
            ...
            logger.error(f"Failed to read JSON from {bucket}/{key}: {str(e)}")
            return None
