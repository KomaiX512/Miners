...
"""
    ...
    def _generate_competitor_analysis_rag(self, session_key: str, primary_username: str,
                                        platform: str) -> Dict:
        """
        BULLETPROOF: Generate competitor analysis using RAG with vector database queries.
        Ensures data is indexed before querying and provides robust fallback mechanisms.
        """
        try:
            ...
            for competitor, data in competitors.items():
                # BULLETPROOF: First ensure competitor data is indexed in vector DB
                posts_content = []
                ...
                try:
                    ...
                    # Index competitor posts directly with proper metadata
                    competitor_posts = data.get('posts', [])
                    if competitor_posts:
                        logger.info(f"🔄 BULLETPROOF: Indexing {len(competitor_posts)} posts for {competitor} in vector DB")
                        
                        indexed_count = 0
                        for i, post in enumerate(competitor_posts[:20]):  # Limit for performance
                            content = post.get('content', '')
                            if content and len(content.strip()) > 10:
                                try:
                                    vector_db.collection.add(
                                        ...
                                        metadatas=[{
                                            ...
                                            'source': 'zero_post_rag_indexing'
                                        }],
                                        ids=[f"zpr_{competitor}_{primary_username}_{i}"]
                                    )
                                    indexed_count += 1
                                except Exception as idx_e:
                                    logger.warning(f"⚠️ Failed to index post {i} for {competitor}: {idx_e}")
                        
                        logger.info(f"✅ Successfully indexed {indexed_count} posts for {competitor}")
                ...
                # CRITICAL DEBUG: Log actual content being analyzed
                logger.info(f"🔍 COMPETITOR CONTENT for {competitor}:")
                logger.info(f"  - Posts retrieved: {len(posts_content)}")
                logger.info(f"  - Content preview: {competitor_content[:200]}...")
...
"""
                ...
                try:
                    logger.info(f"🎯 Generating Gemini analysis for {competitor}...")
                    response = self._safe_generate(analysis_prompt)
                    
                    # CRITICAL DEBUG: Log raw response
                    logger.info(f"🔍 RAW GEMINI RESPONSE for {competitor}:")
                    logger.info(f"  - Response length: {len(response.text) if response.text else 0} chars")
                    logger.info(f"  - Response preview: {response.text[:300] if response.text else 'EMPTY'}...")
                    
                    analysis_json = self._extract_json_from_response(response.text)
                    
                    if analysis_json:
                        logger.info(f"✅ Successfully parsed JSON analysis for {competitor}")
                        ...
                        logger.warning(f"⚠️ Failed to parse JSON from Gemini response for {competitor}, using fallback")
                        ...
                        fallback_analysis['intelligence_source'] = 'rag_competitor_analysis'
