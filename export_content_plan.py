...
class ContentPlanExporter:
    ...
    def export_content_plan(self, content_plan):
        ...
        try:
            ...
            if next_post_data:
                # ✅ DEDUPLICATION: Only prevent exporting when an IDENTICAL next post already exists.
                # Do NOT block just because a file exists or lacks a status field.
                skip_due_to_duplicate = False
                try:
                    existing_next_posts = self.r2_storage.list_files(base_dirs['next_posts'], bucket='tasks')
                except Exception as e:
                    existing_next_posts = []
                    logger.warning(f"⚠️ Could not list existing next_posts for duplicate check: {e}")

                # Prepare a deterministic signature of the content we plan to export
                try:
                    candidate_signature = json.dumps(next_post_data, sort_keys=True)
                except Exception:
                    # If for any reason serialization fails, fall back to always exporting
                    candidate_signature = None

                if existing_next_posts and candidate_signature is not None:
                    inspected = 0
                    for key in sorted(existing_next_posts, reverse=True):
                        if not key.endswith('.json'):
                            continue
                        inspected += 1
                        try:
                            data = self.r2_storage.read_json(key, bucket='tasks')
                        except Exception as e:
                            logger.warning(f"⚠️ Could not read {key} for duplicate check: {e}")
                            continue
                        if not isinstance(data, dict):
                            continue
                        try:
                            existing_signature = json.dumps(data, sort_keys=True)
                        except Exception:
                            continue
                        if existing_signature == candidate_signature:
                            skip_due_to_duplicate = True
                            logger.warning(f"♻️ Identical next_post already exists for {platform}/{primary_username}: {key} – skipping duplicate export")
                            break
                        # Limit deep scans
                        if inspected >= 20:
                            break

                if not skip_due_to_duplicate:
                    ...
                    # Export the clean next post payload
                    ...
                    export_results['next_post'] = False
                    logger.info(f"⏭️ Skipping next post export due to identical content already present for {platform}/{primary_username}")
            else:
