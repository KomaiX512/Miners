...
class ContentRecommendationSystem:
    ...
    def __init__(self):
        ...
        # Initialize elite News For You module with INTELLIGENT filtering
        from intelligent_news_for_you import IntelligentNewsForYouModule
        from config import GEMINI_CONFIG
        
        # Combine R2 and Gemini configs for the news module
        combined_config = {**R2_CONFIG, **GEMINI_CONFIG}
        
        self.news_for_you = IntelligentNewsForYouModule(
        ...
        logger.info("✅ Elite News For You Module initialized for premium news curation")
    ...
    def process_instagram_data(self, raw_data, account_info=None, authoritative_primary_username=None):
        ...
        try:
            ...
            # Handle account info from info.json file with fallback for private accounts
            ...
            if account_info and isinstance(account_info, dict) and account_type_from_info and posting_style_from_info:
                # Use authoritative values from info.json if available
                ...
                # FALLBACK: Handle missing account info for private accounts
                # Check if account is private to provide appropriate defaults
                is_private = account_data.get('private', False)
                
                if is_private:
                    # Private account defaults
                    account_type = 'personal'
                    posting_style = 'Personal social media account focused on sharing life updates and connecting with friends and family.'
                    logger.warning(f"⚠️ PRIVATE ACCOUNT FALLBACK: Using defaults for private account {primary_username} - account_type: {account_type}")
                else:
                    # Public account defaults
                    account_type = 'personal'
                    posting_style = 'Social media account focused on sharing content and building community engagement.'
                    logger.warning(f"⚠️ PUBLIC ACCOUNT FALLBACK: Using defaults for account {primary_username} - account_type: {account_type}")
                
                logger.warning(f"⚠️ Missing account info handled with fallback values - account_type: {account_type}, posting_style: {posting_style}")
    ...
    def generate_content_plan(self, data, topics=None, n_recommendations=3):
        ...
        try:
            ...
            try:
                ...
                if news_for_you_data:
                    content_plan["news_for_you"] = news_for_you_data
                    logger.info(f"✅ Elite News For You module generated with {news_for_you_data.get('premium_summary', {}).get('total_stories', 0)} curated stories")
                else:
    ...
    def _extract_detailed_competitor_insights(self, rag_content, competitor_name):
        ...
        try:
            ...
            # Add industry-standard competitive strengths if none found
            if not competitor_strengths:
                competitor_strengths = [
                    f"Market intelligence reveals {competitor_name}'s competitive positioning and audience engagement strengths"
                ]
            ...
            # Add strategic vulnerability analysis if none found
            if not competitor_vulnerabilities:
                competitor_vulnerabilities = [
                    f"Strategic differentiation opportunities identified through {competitor_name}'s market gap analysis"
                ]
            ...
            logger.warning(f"⚠️ Error extracting insights for {competitor_name}: {str(e)}")
            # Return enhanced minimal structure
            return {
                "overview": f"Competitive intelligence analysis reveals {competitor_name}'s market positioning and strategic content approach",
                "strengths": [f"Market intelligence reveals {competitor_name}'s competitive positioning and audience engagement strengths"],
                "vulnerabilities": [f"Strategic differentiation opportunities identified through {competitor_name}'s market gap analysis"], 
                "recommended_counter_strategies": [f"Strategic market differentiation approach to outperform {competitor_name}"],
                "themes": [],
                "recommendations": []
            }
    ...
    def _generate_news_for_you_module(self, username: str, platform: str, 
        ...
        try:
            ...
            # Use the news module to generate curated news
            import asyncio
            
            # Generate news using the NewsForYouModule (synchronous version)
            news_data = self.news_for_you.generate_news_for_account_sync(
                username=username,
                platform=platform,
                account_type=account_type,
                posting_style=posting_style,
            ...
            # Export the news to R2 storage with proper structure
            if news_data and news_data.get('premium_summary'):
                self._export_news_for_you_to_r2(news_data, username, platform)
            
            logger.info(f"✅ News For You module generated successfully for @{username}")
            return news_data
            ...
            # Return fallback news structure
            return {
                'username': username,
                'platform': platform,
    ...
    def _export_news_for_you_to_r2(self, news_data: Dict, username: str, platform: str):
        """
        Export News For You data to R2 storage with proper directory structure.
        
        Structure: news_for_you/{platform}/{username}/news_{identifier}.json
        """
        try:
            logger.info(f"📤 Exporting News For You to R2 for @{username} on {platform}")
            
            # Generate unique identifier for this news export
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            identifier = f"{timestamp}_{username}"
            ...
            export_data = {
                ...
                'export_metadata': {
                    ...
                    'news_count': news_data.get('premium_summary', {}).get('total_stories', 0),
                    'quality_score': news_data.get('curation_metadata', {}).get('quality_score', 0.0),
                    'export_version': '1.0'
                }
            }
    ...
    def _generate_next_post_module(self, posts, username, platform, main_recommendation=None):
        """Generate the next post prediction module using a separate, dedicated RAG request for maximum quality."""
        try:
            logger.info(f"🎯 Generating DEDICATED next post prediction for {username} on {platform}")
            
            # Store current processed data for hashtag cleaning
            if hasattr(self, '_current_processed_data'):
                self._current_processed_data = getattr(self, '_current_processed_data', {})
            ...
            # Create a DEDICATED prompt for next post prediction only
            next_post_prompt = self._create_dedicated_next_post_prompt(posts, username, platform, account_type, posting_style)
            
            # Make a separate RAG request specifically for next post prediction
            logger.info("📡 Making DEDICATED RAG request for next post prediction...")
            
            try:
                # Configure the RAG system for dedicated request
                if not hasattr(self, 'rag') or not self.rag:
                    logger.error("RAG system not available for dedicated request")
                ...
                generation_config = {
                    ...
                    'top_p': 0.9,
                    'top_k': 40,
                    'max_output_tokens': 800  # Smaller focused response
                }
                
                # Apply rate limiting
                self.rag.rate_limiter.wait_if_needed()
                
                response = self.rag.generative_model.generate_content(
                    contents=next_post_prompt,
                    generation_config=generation_config
                )
                
                self.rag.rate_limiter.record_success()
                
                if not response or not hasattr(response, 'text') or not response.text.strip():
                    raise Exception("Empty response from dedicated next post RAG")
                
                logger.info(f"✅ DEDICATED RAG response received: {len(response.text)} chars")
                
                # Parse the dedicated response
                next_post_data = self._parse_dedicated_next_post_response(response.text, platform)
                ...
                if next_post_data:
                    ...
                    logger.info(f"✅ DEDICATED next post prediction generated successfully for {username}")
                    return next_post_data
                else:
                    logger.error("Failed to parse dedicated RAG response")
                    raise Exception("Failed to parse dedicated response")
                
            except Exception as rag_error:
                logger.error(f"❌ DEDICATED RAG request failed: {str(rag_error)}")
                # 🚨 CRITICAL FIX: No fallback to recommendation data to prevent contamination
                logger.error("🚨 NO FALLBACK: Dedicated next post RAG failed - this ensures no contamination")
                raise Exception(f"Dedicated next post generation failed for {username} - no fallback to prevent contamination")
            ...
            for competitor in competitors:
                ...
                if competitor_data:
                    ...
                    if processed_competitor and processed_competitor.get('posts'):
                        ...
                        logger.info(f"✅ Indexed {posts_added} competitor posts for {competitor}")
    ...
    def _process_twitter_account_from_info(self, username, account_info):
        ...
        try:
            ...
            if primary_data:
                ...
                if processed_primary and processed_primary.get('posts'):
                    posts_added = self.vector_db.add_posts(processed_primary['posts'], username, is_competitor=False)
                    logger.info(f"✅ Indexed {posts_added} primary Twitter posts for {username}")
            ...
            for competitor in competitors:
                ...
                if competitor_data:
                    ...
                    if processed_competitor and processed_competitor.get('posts'):
                        posts_added = self.vector_db.add_posts(processed_competitor['posts'], competitor, is_competitor=True)
                        total_competitor_posts += posts_added
                        logger.info(f"✅ Indexed {posts_added} competitor Twitter posts for {competitor}")
    ...
    def _process_facebook_account_from_info(self, username, account_info):
        ...
        try:
            ...
            if primary_data:
                ...
                if processed_primary and processed_primary.get('posts'):
                    posts_added = self.vector_db.add_posts(processed_primary['posts'], username, is_competitor=False)
                    logger.info(f"✅ Indexed {posts_added} primary Facebook posts for {username}")
                else:
                ...
                try:
                    ...
                    if competitor_data:
                        ...
                        if processed_competitor and processed_competitor.get('posts'):
                            posts_added = self.vector_db.add_posts(processed_competitor['posts'], competitor, is_competitor=True)
                            total_competitor_posts += posts_added
                            logger.info(f"✅ Indexed {posts_added} competitor Facebook posts for {competitor}")
                        else:
                ...
                if hasattr(self, 'vector_db') and self.vector_db:
                    ...
                    if vector_results and 'documents' in vector_results and vector_results['documents']:
                        ...
                        if documents and len(documents) > 0 and documents[0]:  # Check if first list has content
                            competitor_posts = []
                            for i, doc in enumerate(documents[0][:10]):  # Limit to 10 posts
                ...
                try:
                    ...
                    if processed_data and isinstance(processed_data, dict):
                        ...
                        if posts and len(posts) > 0:
                            ...
                            logger.info(f"✅ Found {len(posts[:10])} processed posts for {competitor}")
                except Exception as e:
...
def test_export_primary_prophet_analysis():
    ...
    # Use AI to select a representative test username from available data
    ai_domain_intel = AIDomainIntelligence()
    
    # For testing, use a dynamically selected username based on available data
    import numpy as np
    import pandas as pd
    
    # Get available usernames from existing data directories
    available_usernames = []
    for platform in ['instagram', 'twitter', 'facebook']:
        platform_path = platform
        if os.path.exists(platform_path):
            available_usernames.extend([d for d in os.listdir(platform_path) if os.path.isdir(os.path.join(platform_path, d))])
    
    # Use the first available username or default to a generic test case
    primary_username = available_usernames[0] if available_usernames else "test_beauty_brand"
    logger.info(f"Testing with username: {primary_username}")
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "create_content_plan":
            ...
            try:
                ...
                content_system.continuous_processing_loop(sleep_interval=30)  # Check every 30 seconds
            ...
            try:
                ...
                content_system.sequential_multi_platform_processing_loop(sleep_interval=30)
